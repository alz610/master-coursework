...
Метод \lstinline{QsysCMD.UPDT_JSTAT_MTHD} обновляет статус моделей. Парсит JSON статуса и модели вызывает метод \lstinline{_pars_job_json_lsf} для парсинга значений полей JSON статуса.

Метод \lstinline{_update_jstats_lsf} парсит поля с значениями у JSON статуса и записывает ключ 'имя модели' со значением словарь состояния с полями с значениями:
\begin{lstlisting}
{
    "COMMAND":"bjobs",
    "JOBS":1,
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{type_multithreaded.png}
